...
22 项发现中 **17 项已修复**（含全部中severity及以上），5 项为需设计投入、纯美观或非问题。当前 169 项断言跨平台可跑，连续 3 次全绿、退出码 0。
...
| A22 | 高 | **非 ASCII 文件名触发上游 xrdp 解析器 bug，拖死整个剪贴板通道**：`CLIPRDR_FILEDESCRIPTOR` 的跳过长度由 `wcstombs()` 返回值推算，对非 ASCII 名算得太短（上游 issue #1992，同一区域后来在 PR #1996 补了越界检查），于是一个文件列表里只有第一个描述符被正确读出。症状是「第一张图能粘，之后都不行，连文字一起死，重连才恢复」——由使用者在真实环境实测出。中文 Windows 的截图名 100% 命中。[实测] | ✅ 默认先把截图复制成 `%TEMP%` 里的 ASCII 名副本，把副本路径放上剪贴板（内容字节相同）；副本目录保留最新 20 个；`-KeepOriginalName` 可关掉；18 条断言 |
...
- 169 项断言跨平台可跑，3/3 全绿、退出码 0；harness 的配置值取自生产源码而非重声明。
