...
23 项发现中 **17 项已修复**，2 项部分解决（A11 需设计投入；A22 根因未定位但已换用更保真的默认模式，A23 降级为备选模式），4 项为纯美观或非问题。当前 184 项断言跨平台可跑，连续 3 次全绿、退出码 0。
...
- 184 项断言跨平台可跑，3/3 全绿、退出码 0；harness 的配置值取自生产源码而非重声明。
